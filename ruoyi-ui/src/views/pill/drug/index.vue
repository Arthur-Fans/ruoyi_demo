    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="药品类型" prop="drugType">
        <el-select v-model="queryParams.drugType" placeholder="请选择药品类型" clearable>
          <el-option
            v-for="dict in dict.type.pill_drug_dtype"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="drugList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="药品类型" align="center" prop="drugType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.pill_drug_dtype" :value="scope.row.drugType"/>
        </template>
      </el-table-column>
      <el-table-column label="处方类型" align="center" prop="prescriptionType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.pill_drug_ptype" :value="scope.row.prescriptionType"/>
        </template>
      </el-table-column>
...
export default {
  name: "Drug",
  dicts: ['pill_drug_ptype', 'pill_drug_dtype'],
  data() {
    return {
      // 遮罩层
      loading: true,
